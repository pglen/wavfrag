...
/* =====[ Fft1.h ]========================================== 
   ...
   Description:     The daco project, implementation of the Fft1.h                
...
public:
	...
	// Construct it
    CFft (unsigned points, unsigned sampleRate = 44100);

	// Hard work
    void    Transform (std::vector<int> const & data);

	// Output: from i = 0 to points-1
    inline double  GetIntensity (unsigned i) const
		{ 
        assert (i < _points);
	...
	// Output: from i = 0 to points-1
    inline double  GetPhase (unsigned i) const
		{ 
        assert (i < _points);
        return std::arg (_X[i]);
		}
...
protected:
	...
	void xDataIn (std::vector<int> const & data);

    inline void xPutAt (unsigned i, double val)
	    {
