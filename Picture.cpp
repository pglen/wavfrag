...
/* =====[ Picture.cpp ]========================================================== 
   ...
   Description:     The wavfrag project, implementation of the Picture.cpp                
...

{
	...
	if (m_pPicture == NULL)
	{
		BYTE*	pBuff = NULL;
		int		nSize = 0;
		if (_GetResource(sResource.GetBuffer(0), sResourceType.GetBuffer(0), pBuff, nSize))
		{
			if (nSize > 0)
			{
				...
				if (_GetResource(sResource.GetBuffer(0), sResourceType.GetBuffer(0), pBuff, nSize))
				{
					if (xLoadFromBuffer(pBuff, nSize))
						bResult = true;
				}
...
bool CPicture::Load(CString sFileName)
{
	...
	if (m_pPicture == NULL)
	{
		...
		if (cFile.Open(sFileName, CFile::modeRead | CFile::typeBinary, &e))
		{
			...
			{
				if (xLoadFromBuffer(pBuff, cFile.GetLength()))
					bResult = true;
			}
...
bool CPicture::Draw(CDC* pDC)
{
	if (m_pPicture != NULL)
	{
		...
		return xDraw(pDC, 0, 0, nWidth, nHeight);
	}
	return false;
}

bool CPicture::Draw(CDC* pDC, int xx, int yy)
{
	if (m_pPicture != NULL)
	{
		long hmWidth;
		...
		return xDraw(pDC, xx, yy, nWidth, nHeight);
	}
...
bool CPicture::Draw(CDC* pDC, CPoint Pos)
{
	if (m_pPicture != NULL)
	{
		...
		return xDraw(pDC, Pos.x, Pos.y, nWidth, nHeight);
	}
	return false;
}
...

{
	...
	return xDraw(pDC, x, y, nWidth, nHeight);
...
bool CPicture::xDraw(CDC* pDC, int x, int y, int cx, int cy)
{
	long hmWidth;
	long hmHeight;
	m_pPicture->get_Width(&hmWidth);
...
bool CPicture::_GetResource(LPSTR lpName, LPSTR lpType, void* pResource, int& nBufSize)
{ 
	HRSRC		hResInfo;
	HANDLE		hRes;
	HMODULE		hInst	= NULL; 
