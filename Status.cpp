...

{
	...
	//ICONINFO ci;	HCURSOR cur = GetCursor();
	//BOOL GetIconInfo(cur, &ci);

	SetWindowPos(&wndTopMost, pt.x + gap, pt.y + gap, 0, 0, 
...
void CStatus::PosStatus(CWnd *wnd)
...

{
	...
	//AP2N("CStatus::CenterStatus top=%d left=%d right=%d bottom=%d\r\n", rc.top, rc.left, rc.right, rc.bottom);

	want_x = (rc.right - rc.left) / 2;
	want_y = (rc.bottom - rc.top) / 8;
